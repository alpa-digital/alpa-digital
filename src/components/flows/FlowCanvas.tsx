...
/* ---------- Versión móvil: el mismo flujo apilado en vertical ---------- */

const Connector = ({ animate, color = "#60A5FA" }: { animate: boolean; color?: string }) => (
  <div className="relative h-7 w-full flex justify-center" aria-hidden="true">
    <div className="w-px h-full border-l border-dashed" style={{ borderColor: color, opacity: 0.6 }} />
    {animate && <span className="absolute top-0 w-2 h-2 rounded-full flow-packet-down" style={{ background: color, boxShadow: `0 0 8px ${color}` }} />}
  </div>
);

export const NodeCard = ({ node, animate, compact = false }: { node: FlowNode; animate: boolean; compact?: boolean }) => {
  const style = nodeStyle[node.kind];
  const Icon = flowIcons[node.icon];
  const isAgent = node.kind === "agent";
  return (
    <div
      className={`relative flex items-center gap-3 rounded-xl border ${compact ? "px-3 py-2.5" : "px-3.5 py-3"} ${isAgent && animate ? "shadow-[0_0_28px_rgba(59,130,246,0.35)]" : ""}`}
      style={{ background: style.fill, borderColor: style.stroke, borderStyle: style.dash ? "dashed" : "solid" }}
    >
      <div className="w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0" style={{ background: style.iconBg }}>
      ...
      <span className="absolute top-1.5 right-2.5 text-[8px] tracking-wide" style={{ color: isAgent ? "#93C5FD" : "rgba(255,255,255,0.35)" }}>
        {kindLabel[node.kind].toUpperCase()}
      </span>
    </div>
  );
...
export const FlowStack = ({ flow, animate }: { flow: AreaFlow; animate: boolean }) => {
  const groups: FlowNode["kind"][] = ["trigger", "agent", "tool", "human", "output"];
  const byKind = groups.map((kind) => flow.nodes.filter((n) => n.kind === kind)).filter((list) => list.length > 0);
  return (
    <div className="px-4 pt-3 pb-1">
      {byKind.map((list, gi) => {
        ...
        return (
          <div key={kind}>
            {gi > 0 && <Connector animate={animate} color={kind === "human" ? "#FBBF24" : "#60A5FA"} />}
            {list.length === 1 ? (
              <NodeCard node={list[0]} animate={animate} />
            ) : (
              <div className="grid grid-cols-1 gap-2">
                {list.map((node) => (
                  <NodeCard key={node.id} node={node} animate={animate} compact />
                ))}
              </div>
            )}
          </div>
