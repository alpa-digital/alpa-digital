...
const nodeStyle: Record<FlowNode["kind"], { fill: string; stroke: string; text: string; sub: string; icon: string; dash?: string }> = {
  trigger: { fill: "#141518", stroke: "rgba(255,255,255,0.18)", text: "#FFFFFF", sub: "rgba(255,255,255,0.55)", icon: "#E5E7EB" },
  agent: { fill: "rgba(0,102,255,0.16)", stroke: "#3B82F6", text: "#FFFFFF", sub: "#BFDBFE", icon: "#BFDBFE" },
  tool: { fill: "#141518", stroke: "rgba(255,255,255,0.14)", text: "#FFFFFF", sub: "rgba(255,255,255,0.5)", icon: "#E5E7EB" },
  human: { fill: "#121316", stroke: "rgba(251,191,36,0.6)", text: "#FDE68A", sub: "rgba(253,230,138,0.6)", icon: "#FDE68A", dash: "4 4" },
  output: { fill: "rgba(34,197,94,0.12)", stroke: "rgba(74,222,128,0.7)", text: "#FFFFFF", sub: "rgba(187,247,208,0.8)", icon: "#86EFAC" },
};
...
const FlowCanvas = ({ flow, animate }: { flow: AreaFlow; animate: boolean }) => {
  ...
  return (
    <svg viewBox="0 0 920 380" className="w-full h-auto min-w-[760px] md:min-w-0" role="img" aria-label={`Flujo de automatización: ${flow.title}`}>
      <defs>
        ...
        return (
          <g key={`${flow.id}-${node.id}`} transform={`translate(${node.x} ${node.y})`}>
            <g className={animate ? "flow-node-pop" : undefined} style={{ animationDelay: `${i * 0.1}s` }}>
              ...
              <Icon x="21" y={h / 2 - 9} width="18" height="18" stroke={style.icon} strokeWidth={1.8} />
